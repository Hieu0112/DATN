...
file3 = "Data_Collect/dataEnglish/Train_English/Train1.csv"


csv_file = "Data_Collect/dataEnglish/Details_English/Data_2.csv"
csv_file_data = "Data_Collect/dataEnglish/Train_English/Train2.csv"
...
def Update_label(csv_file, csv_file_data):
    ...
    for row in rows:
        ...
        if so_luong <=20000:
            ...
            check_2.add(title_update)
            check_3.add(text_update)
            if len(text) > 2 * len(title) and len(text) >= 100 and len2+1==len(check_2)and len3+1==len(check_3) and label!='2':
                article_data = {
                    'title': title_update,
                    'text': text_update,
                    'label': label
