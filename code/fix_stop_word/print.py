...
# In danh sách stop words sau khi token hóa
print("Danh sách stop words sau khi token hóa:", tokenized_stopwords)

# Thêm các từ ghép bị chia nhỏ vào danh sách stop words nếu cần
additional_stopwords = ['kỳ', 'xuất']  # Các từ bị token hóa cần thêm vào
tokenized_stopwords.extend(additional_stopwords)

# In lại danh sách stop words sau khi thêm các từ ghép
print("Danh sách stop words cuối cùng:", tokenized_stopwords)
