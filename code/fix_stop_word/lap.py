...
with open('code/fix_stop_word/vietnamese-stopwords.txt', 'r', encoding='utf-8') as file:
    ...
    stopWords = list(set(stopWords))  # Loại bỏ các từ dừng trùng lặp
...
# Loại bỏ các stopwords trùng lặp trong danh sách tokenized
unique_stopwords = list(set(tokenized_stopwords_flat))

# Sử dụng CountVectorizer với stop words đã token hóa và tokenizer
vectorizer = CountVectorizer(
    stop_words=unique_stopwords,  # Áp dụng stopword đã token hóa và loại bỏ trùng lặp
    tokenizer=tokenize,
...
# In ra danh sách stopwords đã token hóa, loại bỏ trùng lặp
print("Danh sách stopwords đã token hóa (loại bỏ trùng lặp):")
print(unique_stopwords)
print(len(unique_stopwords))